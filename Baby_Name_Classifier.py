...
def naivebayesPXY(X,Y):
    ...
    # YOUR CODE HERE
    return np.mean(X[Y == 1], axis=0), np.mean(X[Y == -1], axis=0)
...
def loglikelihood(posprob, negprob, X_test, Y_test):
    ...
    # YOUR CODE HERE
    loglikelihood[Y_test == -1] = np.dot(X_test[Y_test == -1],np.log(negprob)) + np.dot((1 - X_test[Y_test == -1]),np.log(1 - negprob))
    loglikelihood[Y_test == 1] = np.dot(X_test[Y_test == 1],np.log(posprob)) + np.dot((1 - X_test[Y_test == 1]),np.log(1 - posprob))
...
def naivebayes_pred(pos, neg, posprob, negprob, X_test):
    ...
    # YOUR CODE HERE
    ratio = np.log(pos) - np.log(neg) + loglikelihood_grader(posprob, negprob, X_test, np.ones(n)) - loglikelihood_grader(posprob, negprob, X_test, -np.ones(n))
    pred = - np.ones(n)
    pred[ratio > 0] = 1
    return pred
